...
export class DetailsListDocumentsExamplePaginated extends React.Component<
  {
...
> {
  ...
  public render() {
    ...
    return (
      <div>
        ...
        {(this.state.paginatedItems.length>0)? <div>
          ...
          <Pagination
                ...
                onChange={(page) => this._getPage(page)
                          ...
                          />
        </div> : <div style={{margin: "10px", fontSize: "large"}}>Aucun élément à afficher</div>
                         }
  ...
  public componentDidMount(): void  {if (this.props.listID && this.props.siteCollection) {
    ...
    if (this.state.listItems.length > 0) {
      ...
      this.setState({ paginatedItems: newArr }, () =>
        console.log(this.state.paginatedItems)
      );
      this._getPage(1)
      this._getPage(2)
...
setupListItems(siteCollection: string, listID: string, userId: string) {
    ...
    SharepointService.getListItems(siteCollection, listID, userId)
      .then((result) => {
        ...
        this.setState({paginatedItems: this.items.slice(0, this.state.perPage)})
        this.setState(
            {
              totalPages: Math.ceil(
                this.state.listItems.length / this.state.perPage
  ...
  private _onColumnClick = (
    ...
    let newItems: IDocument[] = this.state.paginatedItems.slice();
