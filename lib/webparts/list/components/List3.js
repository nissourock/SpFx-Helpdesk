...
var DetailsListDocumentsExamplePaginated = (function (_super) {
    __extends(DetailsListDocumentsExamplePaginated, _super);
    function DetailsListDocumentsExamplePaginated(props) {
        ...
        _this._onColumnClick = function (ev, column) {
            ...
            var newItems = _this.state.paginatedItems.slice();
    ...
    DetailsListDocumentsExamplePaginated.getDerivedStateFromProps = function (nextProps, prevState) {
        if (prevState.items !== nextProps.items) {
            return { items: nextProps.items };
        }
        return null;
    };
    DetailsListDocumentsExamplePaginated.prototype.createComment = function (siteCollection, commentBody, ticketId, userID) {
    ...
    DetailsListDocumentsExamplePaginated.prototype.deleteComment = function (siteCollection, commentId) {
        var _this = this;
    ...
    DetailsListDocumentsExamplePaginated.prototype.render = function () {
        var _this = this;
        ...
        return (React.createElement("div", null,
            ...
            (this.state.paginatedItems.length > 0) ? React.createElement("div", null,
                React.createElement(MarqueeSelection, { selection: this._selection },
                    React.createElement("div", { style: { marginTop: "30px" } }),
                    React.createElement(DetailsList, { items: this.state.paginatedItems, compact: isCompactMode, columns: columns, selectionMode: this.state.isModalSelection
                            ? SelectionMode.multiple
                            : SelectionMode.none, setKey: this.state.items, layoutMode: DetailsListLayoutMode.justified, isHeaderVisible: true, selection: this._selection, selectionPreservedOnEmptyClick: true, onItemInvoked: this._onItemInvoked, enterModalSelectionOnTouch: true, onActiveItemChanged: this._onActiveItemChanged })),
                React.createElement(Pagination, { currentPage: this.state.selectedPage, totalPages: this.state.totalPages, onChange: function (page) { return _this._getPage(page); } })) : React.createElement("div", { style: { margin: "10px", fontSize: "large" } }, "Aucun \u00E9l\u00E9ment \u00E0 afficher"),
            React.createElement("div", null)));
    };
    DetailsListDocumentsExamplePaginated.prototype._getPage = function (page) {
        var _this = this;
        this.setState({ selectedPage: page });
        this.setState({ offset: (page - 1) * this.state.perPage }, function () { return console.log("offset", _this.state.offset); });
        console.log(this.state.perPage);
        // this.setState({totalPages: Math.ceil(this.state.listItems.length / 3)},()=> console.log(this.state.paginatedItems))
        var newArr = this.state.listItems.slice(this.state.offset, this.state.offset + this.state.perPage);
        ...
        this.setState({ paginatedItems: this.state.listItems.slice(this.state.offset, this.state.offset + this.state.perPage) }, function () {
            return console.log(_this.state.paginatedItems);
        });
        console.log(page);
    };
    DetailsListDocumentsExamplePaginated.prototype.componentDidUpdate = function (previousProps, previousState) {
        var _this = this;
        ...
        if (previousState.selectedPage !== this.state.selectedPage) {
            ...
            console.log(this.state.perPage);
            // this.setState({totalPages: Math.ceil(this.state.listItems.length / 3)},()=> console.log(this.state.paginatedItems))
            var newArr_1 = this.state.listItems.slice(this.state.offset, this.state.offset + this.state.perPage);
            // this.state.listItems.map((item,index)=> { if((index >= this.state.offset )&&(index <= this.state.offset + this.state.perPage)){newArr.push(item)}})
            var filteredArray = this.state.listItems.filter(function (x) {
    ...
    DetailsListDocumentsExamplePaginated.prototype.componentDidMount = function () {
        var _this = this;
        if (this.props.listID && this.props.siteCollection) {
            ...
            var newArr_2 = this.state.listItems.slice(0, this.state.perPage);
            var filteredArray = this.state.listItems.filter(function (x) {
                return newArr_2.indexOf(x) < 0;
            });
            this.setState({ paginatedItems: newArr_2 }, function () {
                return console.log(_this.state.paginatedItems);
            });
            this._getPage(1);
            this._getPage(2);
    ...
    DetailsListDocumentsExamplePaginated.prototype.setupListItems = function (siteCollection, listID, userId) {
        var _this = this;
        this.items = [];
        SharepointService.getListItems(siteCollection, listID, userId)
            ...
            _this.setState({ paginatedItems: _this.items.slice(0, _this.state.perPage) });
            _this.setState({
                totalPages: Math.ceil(_this.state.listItems.length / _this.state.perPage),
            }, function () { return console.log("total pages", _this.state.totalPages); });
        })
            .catch(function (err) { return _this.setState({ listItems: [] }); });
    };
    DetailsListDocumentsExamplePaginated.prototype._onItemInvoked = function (item) {
        console.log("Item invoked: " + item.name);
    };
    DetailsListDocumentsExamplePaginated.prototype._randomDate = function (start, end) {
    ...
    DetailsListDocumentsExamplePaginated.prototype._randomFileIcon = function () {
    ...
    DetailsListDocumentsExamplePaginated.prototype._randomFileSize = function () {
    ...
    DetailsListDocumentsExamplePaginated.prototype._getSelectionDetails = function () {
    ...
    return DetailsListDocumentsExamplePaginated;
}(React.Component));
export { DetailsListDocumentsExamplePaginated };
